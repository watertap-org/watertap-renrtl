...
def populate_enrtl_state_vars_multi(blk, base="FpcTP"):
    ...
    if base == "FpcTP":
        feed_flow_mass = 0.15  # kg/s
        feed_mass_frac_comp = {"Na+": 0.001207, "Cl-": 0.000623, "SO4_2-": 0.00169}
        feed_mass_frac_comp["H2O"] = 1 - sum(x for x in feed_mass_frac_comp.values())
        mw_comp = {
            "H2O": 18.015e-3,
            "Na+": 22.990e-3,
...
def set_scaling(m):
    ...
    m.fs.properties_vapor.set_default_scaling(
        "flow_mass_phase_comp", 1e1, index=("Vap", "H2O")
    )
    m.fs.properties_vapor.set_default_scaling(
        "flow_mass_phase_comp", 1e1, index=("Liq", "H2O")
    )
        ...
        if solve_nonideal:
            ...
            if run_multi:
                ...
                sum_tds_brine_out = sum(
                    value(
                        m.fs.enrtl_state[i].properties[0].flow_mass_phase_comp["Liq", j]
                    )
                    for j in m.fs.set_ions_multi
                )
                print(
...
def model_analysis(m, water_rec=None):
    ...
    def performance_ratio(b):
        return (
            ...
            * 2319.05
        ) / (b.steam_generator.heat_duty[0] / 1000)
    ...
    def eq_specific_energy_consumption(b):
        return b.specific_energy_consumption == (
            pyo.units.convert(
                b.steam_generator.heat_duty[0], to_units=pyunits.kW  # in Watts
            )
            / pyo.units.convert(
