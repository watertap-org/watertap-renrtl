...
liquid/electrolyte phases with water as solvent and NaCl and Na2SO4 as
electrolyte. If you need to model multi-electrolyte please ask the author 
for further help!

This method is a modified version of the refined ENRTL activity
...
#############################################################################
References:
[1] Y. Song, C. C. Chen, Symmetric Electrolyte Nonrandom
...
[3] X. Yang, P. I. Barton, G. M. Bollas Refined electrolyte-NRTL model: 
...
*KEY LITERATURE[3].
*It contains most of the key parameter values and equations.
...
Author: Pengfei Xu from University of Connecticut, Soraya Rawlings from Sandia in collaboration with Wajeha
Tauqir from University of Connecticut

Data and model provided by Prof. Bollas and his research
group from University of Connecticut
...
class rENRTL(Ideal):
    ...
    def common(b, pobj):
        ...
        # Eqn 1 & 5 from ref [7]. "rule_vol_mol_solvent" is used to calculate the total volume of solution,
        def rule_vol_mol_solvent(b):
            ...
            if len(b.params.solvent_set) == 1:
                ...
                return (
                    ...
                    + sum(
                        n[e] *
                        # The term below is Eqn 5 from ref [7]
        ...
        # b term
        # kappa is from first line of Eqn 2 from ref [3]
        # get_b = kappa*a_i /I. The I term here is the ionic strength
        def rule_b_term(b):
        ...
        # First line of Eqn 2 from ref [3]
        def rule_kappa(b):
        ...
        b.add_component(
            pname + "_tau2",
            pyo.Expression(rule=rule_tau2, doc="New calculated tau"),
        )

        # Eqn 1 from ref [3] The denominator of the term before the sum term, multiplied by 3
        def rule_A_DH(b):
            eps = getattr(b, pname + "_relative_permittivity_solvent")
            eps0 = Constants.vacuum_electric_permittivity
        ...
        # This equation does not include Born correction
        # There is no direct reference for this term and it is actually partial differentiation of A which comes from Eqn 1 in ref [3]
        # The partial differentiation is written as dA/dN + dA/dV*Vi in this case, Vi is the partial volume of same species as N.
