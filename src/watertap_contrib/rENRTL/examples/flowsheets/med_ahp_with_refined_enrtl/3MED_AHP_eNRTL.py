...
def populate_enrtl_state_vars_gen(blk, base="FpcTP"): #for AHP loop
    ...
    if base == "FpcTP":
        ...
        feed_mass_frac_comp = {"Li+": 0.0043945, 
                               "Br-": 0.0506055} 
        feed_mass_frac_comp["H2O"] = (1 - sum(x for x in feed_mass_frac_comp.values()))
...
def create_model():
    ...
    # Declare unit models 
    m.fs.generator = Evaporator(property_package_feed=m.fs.properties_feed,property_package_vapor=m.fs.properties_vapor)

    m.fs.economizer = HeatExchanger(delta_temperature_callback=delta_temperature_chen_callback,
                                        hot_side_name="tube",
    ...
    m.fs.expansion_valve = PressureChanger(property_package=m.fs.properties_feed, material_balance_type=MaterialBalanceType.componentTotal,
                                 thermodynamic_assumption=ThermodynamicAssumption.pump) 
    
    m.fs.mixer= Mixer(property_package=m.fs.properties_feed, num_inlets=2, momentum_mixing_type=MomentumMixingType.minimize)

    m.fs.absorber = HeatExchanger(delta_temperature_callback=delta_temperature_chen_callback,
                                        hot_side_name="tube",
                                        cold_side_name="shell",
                                        tube={"property_package": m.fs.properties_feed}, #LiBr absorbent enters tube to preheat SW
                                        shell={"property_package": m.fs.properties_feed_sw}, #SW enters the shell
                                        flow_pattern=HeatExchangerFlowPattern.crossflow) 

    m.fs.pump = PressureChanger(property_package=m.fs.properties_feed,
    material_balance_type=MaterialBalanceType.componentTotal, thermodynamic_assumption=ThermodynamicAssumption.pump)
    ...
    m.fs.evaporator = Evaporator(m.fs.set_evaporators,property_package_feed=m.fs.properties_feed_sw, property_package_vapor=m.fs.properties_vapor)
    
    m.fs.condenser = Condenser(m.fs.set_condensers,property_package=m.fs.properties_vapor)

    m.fs.separator = Separator(property_package=m.fs.properties_vapor,outlet_list=["outlet_1", "outlet_2"],split_basis=SplittingType.totalFlow)
    ...
    m.fs.tblock = Translator(inlet_property_package=m.fs.properties_vapor, outlet_property_package=m.fs.properties_feed)
    @m.fs.tblock.Constraint()
    def eq_flow_mass_comp_H2O(b):
            return b.properties_out[0].flow_mass_phase_comp["Liq", "H2O"] == (
                    b.properties_in[0].flow_mass_phase_comp["Liq", "H2O"] + b.properties_in[0].flow_mass_phase_comp["Vap", "H2O"])
    ...
    @m.fs.Constraint(m.fs.set_evaporators,doc="Molal concentration of solute in solvent in mol of TDS/kg of H2O")
    ...
    # AHP Loop
    m.fs.molal_conc_solute_gen = pyo.Var(initialize=2,
                                     bounds=(1e-3, 50),
                                     units=pyunits.mol/pyunits.kg,
                                     doc="Molal concentration of solute")
    
    @m.fs.Constraint(doc="Molal concentration of solute in solvent in mol of TDS/kg of H2O")
    ...
    if solve_nonideal_AHP:
        ...
        # Add activity coefficient as a global variable in each evaporator.
        m.fs.act_coeff_gen = pyo.Var(initialize=1,
                                 units=pyunits.dimensionless,
                                 bounds=(1e-5, 100))
        ...
        m.fs.prop_enrtl_gen = GenericParameterBlock(**LiBr_enrtl_config_FpcTPupt.configuration)
        ...
        #Add the activity coefficient as a parameter with a value of 1
        m.fs.act_coeff_gen = pyo.Param(initialize=1,
                                   units=pyunits.dimensionless)
...
def add_enrtl_method_AHP(m):
    ...
    # Calculate the total mass of the solute molecule and the mass ratio of each ion in the solute molecule.
    m.fs.enrtl_state_gen.mol_mass_ion_molecule = sum(m.fs.ion_coeff_AHP[j]*sb_enrtl_gen.mw_comp[j]
                                                         for j in m.fs.set_ions_AHP)
    ...
    m.fs.enrtl_state_gen.eq_enrtl_pressure = Constraint(
        expr=(
            sb_enrtl_gen.pressure == m.fs.generator.properties_brine[0].pressure
        )
    )
    ...
    def enrtl_flow_mass_ion_comp(b, j):
        return (
            sb_enrtl_gen.flow_mass_phase_comp["Liq", j] == (
                (m.fs.generator.properties_brine[0].flow_mass_phase_comp["Liq", "TDS"]*
                 b.mass_ratio_ion[j])
            )
        )
    m.fs.enrtl_state_gen.enrtl_flow_mass_ion_comp_eq = Constraint(m.fs.set_ions_AHP,
                                                                      rule=enrtl_flow_mass_ion_comp)
    ...
    m.fs.enrtl_state_gen.mean_act_coeff = Expression(
        expr=log(
            (sb_enrtl_gen.act_coeff_phase_comp["Liq", "Li+"] ** m.fs.ion_coeff_AHP["Li+"]*
             sb_enrtl_gen.act_coeff_phase_comp["Liq", "Br-"] ** m.fs.ion_coeff_AHP["Br-"])**
    ...
    m.fs.enrtl_state_gen.conv_mole_frac_to_molal = Expression(
        expr=log(
            1 +
            (sb_enrtl_gen.mw_comp["H2O"]*2*m.fs.molal_conc_solute_gen)/1 # 1 kg of solvent
        )
    )
    m.fs.enrtl_state_gen.molal_mean_act_coeff = Expression(
        expr=(m.fs.enrtl_state_gen.mean_act_coeff -
...
def set_model_inputs(m):
    ...
    m.fs.pump.inlet.temperature.fix(150 + 273.15) #K, set point of heat transfer fluid from solar array to generator in [2] is 180degC
...
def add_bounds(m):
    ...
    for i in m.fs.set_evaporators:
        ...
        m.fs.evaporator[i].outlet_brine.temperature[0].setub(73 + 273.15) # in K to protect pipes
...
def print_results(m):
    ...
    print('                      Area  : {:>4.3f}'.format(
            value(m.fs.generator.area)))
    print('                         U  : {:>4.3f}'.format(
            value(m.fs.generator.U)))
    print('                         Qin: {:>4.3f}'.format(
    ...
    print('   flow_mass_phase_comp (kg/s) {:>15.4f} {:>14.4f} {:>14.4f}'.format(
            value(sw_blk_gen.flow_mass_phase_comp["Liq", "H2O"]
                  + sw_blk_gen.flow_mass_phase_comp["Liq", "TDS"]),
            value(brine_blk_gen.flow_mass_phase_comp["Liq", "H2O"]
...
def model_analysis(m, water_rec=None):
    ...
    def total_water_produced_gpm(b):
        return pyo.units.convert(
            sum(b.condenser[e].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"]
                for e in m.fs.set_condensers)/m.fs.water_density,
    ...
    def performance_ratio(b):
        return ((b.condenser[1].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"] +
              ...
              b.condenser[4].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"]) * 2319.05)/(b.generator.heat_transfer/1000)
    ...
    def eq_specific_energy_consumption(b):
         return b.specific_energy_consumption == (
             pyo.units.convert(b.generator.heat_transfer, #in Watts
                             to_units=pyunits.kW)/
    ...
    def rule_water_recovery(b):
        return m.fs.water_recovery == (
            b.condenser[1].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"] +
            b.condenser[2].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"] +
            b.condenser[3].control_volume.properties_out[0].flow_mass_phase_comp["Liq", "H2O"] +
