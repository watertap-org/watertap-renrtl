...
class TestMED:
    ...
    def test_create_arcs(self, MED_AHP_eNRTL):
        ...
        arc_dict = {
            ...
            m.fs.economizer_to_generator:(m.fs.economizer.shell_outlet,m.fs.generator.inlet_feed),
            m.fs.generator_to_economizer:(m.fs.generator.outlet_brine,m.fs.economizer.tube_inlet),
            m.fs.economizer_to_valve:(m.fs.economizer.tube_outlet,m.fs.expansion_valve.inlet),
            ...
            m.fs.generator_to_condenser:(m.fs.generator.outlet_vapor, m.fs.condenser[1].inlet),
            m.fs.absorber_to_evaporator_feed:Arc(m.fs.absorber.shell_outlet,m.fs.evaporator[1].inlet_feed),
            m.fs.evap1brine_to_evap2feed:Arc(m.fs.evaporator[1].outlet_brine, m.fs.evaporator[2].inlet_feed),
            ...
            m.fs.evap3vapor_to_separator:(m.fs.evaporator[3].outlet_vapor,m.fs.separator.inlet),
            m.fs.separator_to_condenser:(m.fs.separator.outlet_2, m.fs.condenser[4].inlet),
    ...
    def test_set_model_inputs(self, MED_AHP_eNRTL):
        ...
        assert value(m.fs.feed_sw.properties[0].flow_mass_phase_comp["Liq", "H2O"]) == 0.24
        assert m.fs.feed_sw.properties[0].flow_mass_phase_comp["Liq", "TDS"].is_fixed()
        assert value(m.fs.feed_sw.properties[0].flow_mass_phase_comp["Liq", "TDS"]) == 0.0058
        ...
        assert m.fs.economizer.tube_inlet.flow_mass_phase_comp[0, "Liq", "H2O"].is_fixed()
        assert value(m.fs.economizer.tube_inlet.flow_mass_phase_comp[0, "Liq", "H2O"]) == 0.35
        assert m.fs.economizer.tube_inlet.flow_mass_phase_comp[0, "Liq", "TDS"].is_fixed()
        assert value(m.fs.economizer.tube_inlet.flow_mass_phase_comp[0, "Liq", "TDS"]) == 0.65
        ...
        assert m.fs.separator.outlet_1_state[0.0].flow_mass_phase_comp['Liq','H2O'].is_fixed()
        assert value(m.fs.separator.outlet_1_state[0.0].flow_mass_phase_comp['Liq','H2O']) == 0
        assert m.fs.separator.outlet_2_state[0.0].flow_mass_phase_comp['Liq','H2O'].is_fixed()
        assert value(m.fs.separator.outlet_2_state[0.0].flow_mass_phase_comp['Liq','H2O']) == 0
        ...
        assert m.fs.tblock.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].is_fixed()
        assert value(m.fs.tblock.properties_out[0].flow_mass_phase_comp["Liq", "TDS"]) == 0
    ...
    def test_initialize(self, MED_AHP_eNRTL):
        ...
        assert value(m.fs.absorber.overall_heat_transfer_coefficient) == pytest.approx(500 ,rel=1e3)
        assert value(m.fs.absorber.shell_outlet.temperature) == pytest.approx(75 + 273.15, rel=1e3)
        assert value(m.fs.economizer.overall_heat_transfer_coefficient) == pytest.approx(600, rel=1e3)
        assert value(m.fs.generator.outlet_vapor.pressure[0]) == pytest.approx(30000, rel=1e3)
        ...
        assert value(m.fs.evaporator[1].delta_temperature_out) == pytest.approx(2.5, rel=1e3)
        assert value(m.fs.evaporator[2].U) == pytest.approx(1000, 1e3)
        assert value(m.fs.evaporator[2].delta_temperature_out) == pytest.approx(2.5, rel=1e3)
        assert value(m.fs.evaporator[3].U) == pytest.approx(1000, 1e3)
        assert value(m.fs.evaporator[3].delta_temperature_out) == pytest.approx(2.5, rel=1e3)
    ...
    def test_model_analysis(self, MED_AHP_eNRTL):
        ...
        for e in m.fs.set_evaporators:
            assert isinstance(m.fs.eq_upper_bound_evaporators_delta_temprature_in[e], Constraint)
